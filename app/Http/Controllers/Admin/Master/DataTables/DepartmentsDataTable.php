        ...
        return datatables()
            ...
            ->editColumn('status', function ($record) {
                ...
                $html = '<label class="form-check form-switch text-center form-switch-lg">
                    <input class="toggle_switch_chk form-check-input" id="normal' . $record->id . '" type="checkbox" value="' . $status . '" ' . $checkedStatus . ' data-status_id="' . $record->id . '" data-href="' . route('master.departments.status', [$record->id]) . '">
                </label>';
                return $html ?? "";
            })
            ->addColumn('action', function ($record) {
                ...
                if (Gate::check('departments.edit')) {
                    $action .= '<a href="javascript:void(0)" data-href="' . route('master.departments.edit', [$record->id]) . '" class="btn btn-sm btn-success edit-item-btn me-2 editRecordBtn"><i class="ri-edit-box-line align-bottom"></i></a>';
                }

                if (Gate::check('departments.delete')) {
                    $action .= '<button data-href="' . route('master.departments.destroy', [$record->id]) . '" class="btn btn-sm btn-danger remove-item-btn delete_role_button deleteRecordBtn"><i class="ri-delete-bin-line"></i></button>';
                }
