...
class Atompay{
  ...
  public function atomResponse($request){
    //echo '<pre>';print_r($_GET);die;
    $this->setRespHashKey($request->hash_response_key);
  ...
  // CREATE
  public function atom_request($request){
    ...
    $this->setLogin($request->login);
    $this->setPassword($request->password);
    $this->setProductId('KARTAR');
    $this->setAmount($request->amount);
    $this->setTransactionCurrency($request->currency);
    $this->setTransactionAmount($request->transaction_amount??0);
    $this->setReturnUrl($request->return_url);
    $this->setClientCode($request->client_code); //123
    $this->setTransactionId($request->reference);
    $this->setTransactionDate($transactionDate);
    $this->setCustomerName($request->login??'Test');
    $this->setCustomerEmailId($request->email??'');
    $this->setCustomerMobile($request->mobile??'');
    $this->setCustomerBillingAddress($request->address??'');
    $this->setCustomerAccount($request->customer_account??''); //639827
    $this->setReqHashKey($request->hash_request_key); //KEY123657234

    $url = $this->getPGUrl();
   $response= array('info'=>array('data'=>array('link'=>$url)));
    return $response;
  }
}
?>