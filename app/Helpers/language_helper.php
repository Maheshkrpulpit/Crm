...
if (!function_exists('getLangName')) {
    ...
    {       
        if(session()->has('locale')){
            $lang = Language::where('languagecode', session()->get('locale'))->first();
            if($lang != null){
                return $lang->languagename;
            }else{
                $lang = Language::where('languagecode', getLang())->first();
                if($lang != null){
                    return $lang->languagename;
                }
            }
...
if (!function_exists('lang')) {
    function lang($key, $group = null, $lang = null)
    {
        // if (request()->segment() != 'admin') {
            if ($lang == null) {
                $lang = get_general_setting('default_language');
            }
            if($lang == ""){
            ...
            $translation = Translate::where('lang_code', $lang)->where('key', $key)->where('group_langname', $group)->first();
           // dd($translation);

            if ($translation == null) {
                foreach (Language::all() as $language) {
                    $translation = new Translate;
                    $translation->lang_code = $language['code'];
